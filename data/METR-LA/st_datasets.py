...
def load_pems_data(rootpath, name, adj_mx_name, ratio=1.0, one_node=False, one_sample=False):
    ...
    train_edge_index, train_edge_attr = adj_mx_to_torch_sparse_tensor_coo(train_adj_mx_ts)
    eval_edge_index, eval_edge_attr = adj_mx_to_torch_sparse_tensor_coo(eval_adj_mx_ts)
    ...
    train_features = raw_data['train']['x'][..., FEATURE_START:FEATURE_END] #(23974, 12, 207, 1)
    #print(raw_data['train']['x'])  
    #print(raw_data['train']['x'].shape)#(23974, 12, 207, 2)
    ...
    for name in ['train', 'val', 'test']:
        #print(raw_data[name]['y'].shape)
        #print(np.count_nonzero(raw_data[name]['y']))
        #print(torch.count_nonzero((torch.from_numpy(raw_data[name]['y']==0)).float()))
        #input()
        x = feature_scaler.transform(raw_data[name]['x'])
        y = feature_scaler.transform(raw_data[name]['y'])
    ...
    print(torch.sum(adj_mx_ts,dim=1))
    input()
    return loaded_data, adj_mx_ts
