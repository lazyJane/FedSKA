...
from FLAlgorithms.trainmodel.layers import Diag
from FLAlgorithms.trainmodel.utils import *


class FullParam(nn.Module):
    def __init__(self, features, non_linearity, k, knn_metric, i, sparse, device, batch_adj ):
        ...
        if self.non_linearity == "exp":
            self.Adj = nn.Parameter(
                torch.from_numpy(nearest_neighbors_pre_exp(features.cpu(), self.k, self.knn_metric, self.i)))
        elif self.non_linearity == "elu":
            self.Adj = nn.Parameter(
                torch.from_numpy(nearest_neighbors_pre_elu(features.cpu(), self.k, self.knn_metric, self.i)))
        elif self.non_linearity == 'none':
            self.Adj = nn.Parameter(torch.from_numpy(nearest_neighbors(features.cpu(), self.k, self.knn_metric)))
        else:
...
class MLP_Diag(nn.Module):
    def __init__(self, nlayers, isize, k, knn_metric, non_linearity, i, sparse, mlp_act, device, batch_adj):
...
class MLP(nn.Module):
    def __init__(self, nlayers, features, isize, hsize, osize, mlp_epochs, k, knn_metric, non_linearity, i, sparse, mlp_act, device, batch_adj):
    ...
    def mlp_knn_init(self):
        if self.input_dim == self.output_dim:
            ...
            print(self.features.shape)#torch.Size([100, 17, 32, 74]) #torch.Size([100, 32, 64])
            labels = torch.from_numpy(nearest_neighbors(self.features.cpu(), self.k, self.knn_metric)).to(self.device)
    ...
    def forward(self, features):
        if self.sparse:
            ...
            embeddings = self.internal_forward(features) #torch.Size([32, 100, 64])
            embeddings = F.normalize(embeddings, dim=1, p=2)
            similarities = cal_similarity_graph(embeddings)
            similarities = top_k(similarities, self.k + 1)
            similarities = apply_non_linearity(similarities, self.non_linearity, self.i)
